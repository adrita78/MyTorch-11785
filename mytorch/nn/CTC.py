...
class CTC(object):
    ...
    def extend_target_with_blank(self, target):
        ...
        for t in range(N-2):
            current_symbol = extended_symbols[t]
            next_symbol = extended_symbols[t+2]
        ...
        # TODO: initialize skip_connect to be all zeros.
          ...
          # For every pair of non-blank symbols in the extended sequence, mark the middle blank as a skip connection
    ...
    def get_posterior_probs(self, alpha, beta):
        ...
        for t in range(T):
            ...
            gamma_sum = alpha[t] * beta[t]
            gamma_divide = np.sum(gamma_sum)
            gamma[t] = gamma_sum / gamma_divide
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            # TODO
            # <---------------------------------------------
            new_target = target[batch_itr][:target_lengths[batch_itr]]
            new_logits = logits[:input_lengths[batch_itr], batch_itr]
            extended_symbols, skip_connect = self.ctc.extend_target_with_blank(new_target)
            alpha = self.ctc.get_forward_probs(new_logits, extended_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(new_logits, extended_symbols, skip_connect)
            gamma = self.ctc.get_posterior_probs(alpha, beta)
            self.gammas.append(gamma)
            t = 0
            while t < (gamma.shape[0]):
                for k in range(gamma.shape[1]):
                    total_loss[batch_itr] += -gamma[t][k] * np.log(new_logits[t, extended_symbols[k]])
                t +=1 
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            input = CTC(self.BLANK)
            logits_batch = self.logits[0:self.input_lengths[batch_itr], batch_itr]
            extended_symbols, _ = input.extend_target_with_blank(self.target[batch_itr, 0:self.target_lengths[batch_itr]])

            for i in range(gamma.shape[1]):
                dY[0:self.input_lengths[batch_itr], batch_itr, extended_symbols[i]] -= gamma[:, i] / logits_batch[:,extended_symbols[i]]
